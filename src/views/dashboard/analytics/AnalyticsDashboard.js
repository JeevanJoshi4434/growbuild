  ...
  getData = async () => {
    try {
      const res = await axios.get(
        process.env.REACT_APP_PORT + `/api/get/stats`
      );
      if (res.status === 200) {
        this.setState({
          data: res.data,
      ...
      <React.Fragment>
        <Row className="match-height">
          ...
          <Col lg="3" md="6" sm="12">
            <SuberscribersGained
              data={
                this.numberFormatter(this.state.data.customers) || `Loading...`
              }
          ...
          <Col lg="3" md="6" sm="12">
            <OrdersReceived
              data={
                this.numberFormatter(this.state.data.totalUnits) || `Loading...`
              }
        ...
        <Row className="match-height">
          ...
          <Col lg="3" md="6" sm="12">
            <SuberscribersGained
              data={
                this.numberFormatter(this.state.data.bookingStat?.bookings) ||
                `Loading...`
              }
              Icon={<Book className="primary" size={22} />}
              Title={"Bookings"}
            />
          </Col>
          <Col lg="3" md="6" sm="12">
            <OrdersReceived
              data={
                this.numberFormatter(
                  parseInt(
                    this.state.data.bookingStat?.total -
                      this.state.data.bookingStat?.bookings
                  )
                ) || `Loading...`
              }
              Icon={<Package className="warning" size={22} />}
          ...
          <Col lg="3" md="6" sm="12">
            <SuberscribersGained
              data={
                this.numberFormatter(this.state.data.totalPrice) || `Loading...`
              }
              Icon={<FileText className="primary" size={22} />}
              Title={"Total Payment Raised"}
            />
          </Col>
          <Col lg="3" md="6" sm="12">
            <OrdersReceived
              data={
                this.numberFormatter(this.state.data.totalPriceCollected) ||
                `Loading...`
              }
              Icon={<Bell className="warning" size={22} />}
