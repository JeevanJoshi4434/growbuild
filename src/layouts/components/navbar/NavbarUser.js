...
import Autocomplete
  from '../../../components/@vuexy/autoComplete/AutoCompleteComponent';
  ...
  return (
    <DropdownMenu right>
      ...
      >
        ...
        <span onClick={()=>logoutUser()} className="align-middle">Log Out</span>
      </DropdownItem>
    </DropdownMenu>
  );
};
  ...
  render () {
    ...
    return (
      <ul className="nav navbar-nav navbar-nav-user float-right">
                  ...
                  >
                    <div
                      className={classnames ({
                        'd-flex justify-content-between align-items-center': item.file ||
                          item.img,
                      })}
                    >
        ...
        <Icon.Mail className='my-2 cursor-pointer' onClick={() => history.push ('/email/inbox')} size={21} />
        <UncontrolledDropdown
          tag="li"
